...
public class MaximumFillRateRandomInitialPopulationProvider<T> implements InitialPopulationProvider<T> {
    ...
    public MaximumFillRateRandomInitialPopulationProvider(
            int initialSize,
            int rows,
            int columns,
            int fillRate,
        ...
        for (int i = 0; i < initialSize; i++) {
            ...
            for (int j = 0; j < rows; j++) {
                for (int z = 0; z < columns; z++) {
                    if (function.apply(newModel) <= assignmentsAmount)
